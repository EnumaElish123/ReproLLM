...
Expand ReproLLM's standing dogfooding policy from one repository to five pinned
open-source repositories. Every development session must exercise all five with
...
Keep `EleutherAI/lm-evaluation-harness` and add the four other projects recorded in
[`../../../val.md`](../../../val.md):
...
checked out at the commit recorded in `val.md`, and never committed to
ReproLLM. The five repositories form one complementary portfolio rather than five
...
all five repositories. At minimum, run `audit` against every repository. Starting
...
A session cannot be reported complete until all five repositories have a result.
...
Replace the current single-target standing dogfooding section with a concise mandatory pointer to
`val.md`. That validation document is the source of truth containing:

1. the five repository names, locations, pinned commits, and validation roles;
...
6. detailed case rationale, upstream links, and scenario design.
...
- `AGENTS.md` points to `val.md`, which names all five pinned repositories and gives each an
  explicit local path and purpose.
- Every development session has a checkable five-target static gate.
